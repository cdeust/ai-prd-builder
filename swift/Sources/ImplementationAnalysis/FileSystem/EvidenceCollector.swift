...
public struct EvidenceBuilder {
    ...
    public func buildEvidence(
        ...
        basePath: String
    ) -> [ImplementationAnalyzer.Evidence] {
        var evidenceList: [ImplementationAnalyzer.Evidence] = []
        ...
        for pattern in searchPatterns {
            ...
            for file in files {
                ...
                if let snippet = collector.extractSnippet(
                    ...
                    let evidence = ImplementationAnalyzer.Evidence(
                        type: .supporting,
                        location: "\(file):1",
                        snippet: snippet,
        ...
        if evidenceList.isEmpty {
            evidenceList.append(ImplementationAnalyzer.Evidence(
                type: .missing,
                location: basePath,
    ...
    public func verifyPattern(
        ...
        if !matches.isEmpty {
            ...
            return VerificationResult(
                status: .rejected,
                evidence: [ImplementationAnalyzer.Evidence(
                    type: .missing,
    ...
    public struct VerificationResult {
        public let status: ImplementationAnalyzer.Hypothesis.VerificationStatus
        public let evidence: [ImplementationAnalyzer.Evidence]
